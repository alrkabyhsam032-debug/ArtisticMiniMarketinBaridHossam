...
The backup service keeps a local gzip copy and mirrors each enabled backup to a dedicated Google Drive folder through the Replit connector proxy. OAuth tokens are minted/refreshed by the connector identity flow and must never be stored in the repo.
